            ...
            <div className="space-y-6">
              {events.map((event, _) => (
                <div key={event.id} className="relative flex items-start">
                  {/* Timeline Dot */}
                  <div
                    className={`flex-shrink-0 w-12 h-12 rounded-full border-2 flex items-center justify-center text-lg z-10 ${getEventColor(event.type)}`}
                  >
                  ...
                  <div className="ml-6 flex-1">
                    <div className="bg-white/10 p-4 rounded-lg border border-white/20 hover:bg-white/15 transition-colors duration-200">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2">
                        ...
                        <div
                          className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${getEventColor(event.type)}`}
                        >
